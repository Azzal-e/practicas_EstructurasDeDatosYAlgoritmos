...
bool crearPeriodo(const instante& i, const instante& f, periodo& p){
    if(anterior(i, f) && agno(i) == agno(f) && mes(i) == mes(f)){
        p.inicio = i;
        p.final = f;
        return true;
    }
...
*/
bool pAnterior(const periodo& p1, const periodo& p2){
...
bool solapados(const periodo& p1, const periodo& p2){
    return (p1 == p2) || (anterior(p1.inicio, p2.inicio) &&  posterior(p1.final, p2.inicio)) 
    || (posterior(p1.final, p2.final) && anterior(p1.inicio, p2.final)) || (posterior(p1.inicio, p2.inicio) && anterior(p1.final, p2.final));
}
